...
public class GroundTracker : MonoBehaviour
{
    ...
    public void snapAllOnjects() {
        grid = FindObjectOfType<Grid>();
        snapAllOnjects(theGround, new Dictionary<Vector3, GameObject>());
    }
    ...
    private void snapAllOnjects(Transform parent,  Dictionary<Vector3, GameObject> ground) {
        foreach(Transform child in parent) {
            if (child.tag != "GroundeCube" && child.tag != "interactablecube") {
                ...
                snapObject(child, ground);
            }
        }
    }

    private void snapObject(Transform transform, Dictionary<Vector3, GameObject> ground) {
        ...
        updateCubesSurrounding(transform.gameObject, pos, ground);

        ground.Add(pos, transform.gameObject);
    ...
    public void AddCube(Vector3 pos, int type) {
        ...
        updateCubesSurrounding(go, gridPos, ground);
        ground.Add(gridPos, go);
    }
